...
df2 = pur_df %>% group_by(Date= floor_date(pur_df$Date, "month")) %>% summarize(Quantity=sum(pur_df$Quantity), Amount=sum(pur_df$Amount))
df2$Quantity = tapply(pur_df$Quantity, format(pur_df$Date,'%m-%Y'), sum)
df2$Amount = tapply(pur_df$Amount, format(pur_df$Date,'%m-%Y'), sum)
pur_df = df2

df2 = sal_df %>% group_by(Date= floor_date(sal_df$Date, "month")) %>% summarize(Quantity=sum(sal_df$Quantity), Amount=sum(sal_df$Amount))
